...
def obtener_cliente(lista_usuarios, salasd):
...
def entradas(salas, compras, usu_lista):
    ...
    cliente = obtener_cliente(lista_usuarios, sala)
...
def buscar_factura(compras, salas, lista_usuarios):
    print("\n")
    while True:
        try:
...
def reconstruir_salas(compras, salas):
        for c in compras:
            for s in salas:
                if s.nombre == c.sala:
                    if c.asiento in s.posiciones:
                        fila, col = s.posiciones[c.asiento]
...
def main():
    ...
    while True:
        ...
        if op == "1":
            ...
            limpiar()
            titulo()
            buscar_factura(compras, salas_lista, listausuarios)
            limpiarrapido()
        elif op == "5":
            limpiar()
            titulo()
